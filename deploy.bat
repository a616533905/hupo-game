...
for %%f in (nanobot_bridge.py voice-proxy.js start.sh status.sh stop.sh install.sh uninstall.sh config.example.json requirements.txt hupo-bridge.service hupo-voice.service logrotate.conf index.html favicon.ico) do (
    if exist "%%f" (
        echo [Copying] %%f
