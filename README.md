...
| 🏰 **塔楼挑战** | 乱入小怪系统，Boss随机状态（打工/睡觉/特训等） |
| 🗺️ **探索冒险** | 多地图探索，随机事件，NPC 互动，天气系统 |
| 📈 **养成系统** | 等级、装备、皮肤、成就 |
| 🏆 **竞技场** | PVP 对战，排名系统 |
...
- **攻击检测** → 自动封禁IP
- **服务异常** → 自动尝试重启恢复
- **SSH暴力破解** → 检测失败次数，5次封禁1天，10次永久封禁
- **Web登录失败** → Token验证失败5次封禁1天，10次永久封禁

### 登录失败封禁机制

| 来源 | 5次失败 | 10次失败 |
|------|---------|----------|
| SSH 22端口 | 临时封禁1天 | 永久封禁 |
| Web Token验证 | 临时封禁1天 | 永久封禁 |

> ⚠️ 封禁IP会同步到服务器防火墙（iptables），确保生效。
...
### v1.8.0 (最新)
- 🛡️ 安全加固：
  - 新增 Fail2ban 集成，自动封禁攻击IP
  - 新增 Sysctl 内核参数加固（SYN Cookie、ICMP保护等）
  - 新增 UFW 防火墙自动配置
  - HTTP 安全响应头全覆盖（X-Content-Type-Options、X-Frame-Options等）
  - CORS 白名单机制，支持IP地址限制
  - POST 请求体大小验证
- 🐛 Bug修复：
  - 修复战斗buff永久效果问题（塔楼/竞技场/故事战斗）
  - 修复战败后UI不更新问题
  - 修复成就保存可能丢失问题
- 🔧 代码优化：
  - 统一战斗结束buff清理逻辑
...
### v1.7.0
- 🛡️ 安全增强：
  - SSH 22端口日志审计，联动SOAR封禁
  - Web登录失败处理：5次封禁1天，10次永久封禁
  - SSH暴力破解检测：5次失败封禁1天，10次永久封禁
- 🔊 语音播报优化：
  - Edge TTS 失败时自动回退到系统 TTS
  - 改进用户交互检测，提高语音激活稳定性
- 🗺️ 地理位置识别：
  - 安全上下文检测（HTTPS/localhost）
  - 自动区分 GPS 定位和 IP 定位
  - 非安全上下文时使用 IP 定位作为备用方案

### v1.6.0
- 🌙 天气系统优化：夜晚效果改为"遇到高阶怪物概率+30%"
- 🏰 塔楼系统重构：
  - 新增乱入小怪系统（友善/混乱/敌对三种行为）
  - 新增Boss状态系统（打工、睡觉、用餐、度假、特训）
  - Boss特训状态属性增强30%
- 🗺️ 冒险地图优化：
  - 移除区域名称中的"矿场"后缀
  - 新增每个区域的特性描述
